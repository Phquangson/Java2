...
//		add(lblTitle, BorderLayout.NORTH);
		...
		formPanel
				.setBorder(new TitledBorder(null, "Search category", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		formPanel.setBackground(Color.WHITE);

		txtTitle = new JTextField(15);
		JButton btnAdd = new JButton("Add");
		btnAdd.setIcon(new ImageIcon(JPanelCategory.class.getResource("/resources/icon-add.png")));
		styleButton(btnAdd);
		...
		btnClear.setIcon(new ImageIcon(JPanelCategory.class.getResource("/resources/icon-trash.png")));
		...
		DefaultTableModel modelTable = new DefaultTableModel(new Object[] { "No.", "Category", "ID", "Update", "Delete" },
				0) {
			@Override
		...
		btnAdd.addActionListener(e -> {
			...
			JPanelAddCategory panelAddStatus = new JPanelAddCategory(this);
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		private String action;
		private JPanelCategory parent;
		private int row;

		public ButtonEditor(JCheckBox checkBox, String action, JPanelCategory parent) {
			super(checkBox);
			this.action = action;
			this.parent = parent;
			button = new JButton();
			...
			button.addActionListener(e -> {
				...
				if (action.equals("Update")) {
					...
					if (category != null) {
						JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(parent), "Update category",
								true);
						dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

						dialog.setUndecorated(true);

						JPanelAddCategory panelAddStatus = new JPanelAddCategory(parent);
						panelAddStatus.setCategoryData(category);
					...
					int confirm = JOptionPane.showConfirmDialog(parent, "Delete this category?", "Confirm",
							JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						boolean deleted = parent.model.delete(id);
						if (deleted) {
							JOptionPane.showMessageDialog(parent, "Category deleted successfully");
							parent.load();
						} else {
	...
	void load() {
		...
		m.setRowCount(0);
		int stt = 1;
		for (Category cate : model.findAll()) {
			m.addRow(new Object[] { stt++, cate.getTitle(), cate.getId(), "Update", "Delete" });
		}
	}

	private boolean isDuplicate(String title, Integer ignoreId) {
		for (Category cate : model.findAll()) {
			if (cate.getTitle().equalsIgnoreCase(title)) {
				if (ignoreId == null || cate.getId() != ignoreId) {
					return true;
				}
			}
		}
	...
	private void searchAndLoad(String keyword) {
		...
		for (Category cate : model.findAll()) {
			if (cate.getTitle().toLowerCase().contains(keyword.toLowerCase())) {
				m.addRow(new Object[] { stt++, cate.getTitle(), cate.getId(), "Update", "Delete" });
				found = true;
			}
		}
		...
		if (!found) {
			m.addRow(new Object[] { "", "No matching categories found", null, "", "" });
