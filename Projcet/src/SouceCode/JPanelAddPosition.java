...
public class JPanelAddPosition extends JPanel {
	...
	public JPanelAddPosition(JPanelPosition parent) {
		this.parent = parent;
		initUI();
	}
	...
	private void initUI() {
		...
		lblTitle = new JLabel("Add Position", SwingConstants.CENTER);
		...
		JLabel lblStatus = new JLabel("Position:");
		...
		btnAdd.addActionListener(e -> {
			...
			if (!statusText.isEmpty()) {
				...
				if (model.insert(statusText)) {
					...
					JOptionPane.showMessageDialog(this, "Failed to add Position!", "Error", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(this, "Please enter a Position before adding.");
			}
		});
	...
	public void setPositionData(Position position) {
		txtStatus.setText(position.getTitle());
		lblTitle.setText("Update position");
		btnAdd.setText("Update position");
		...
		btnAdd.addActionListener(e -> {
			...
			if (!newTitle.isEmpty()) {
				PositionModel model = new PositionModel();
				if (model.update(position.getId(), newTitle)) {
	...
	private void enableDrag() {
		...
		addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
			...
			public void mouseDragged(java.awt.event.MouseEvent e) {
				Window window = SwingUtilities.getWindowAncestor(JPanelAddPosition.this);
				if (window != null) {
