...
public class JPanelPosition extends JPanel {
...
//		add(lblTitle, BorderLayout.NORTH);
		...
		formPanel.setBorder(
				new TitledBorder(null, "Search position", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		formPanel.setBackground(Color.WHITE);

		txtTitle = new JTextField(15);
		JButton btnAdd = new JButton("Add");
		btnAdd.setIcon(new ImageIcon(JPanelPosition.class.getResource("/resources/icon-add.png")));
		styleButton(btnAdd);
		...
		btnClear.setIcon(new ImageIcon(JPanelPosition.class.getResource("/resources/icon-trash.png")));
		...
		DefaultTableModel modelTable = new DefaultTableModel(
				new Object[] { "No.", "Position", "ID", "Update", "Delete" }, 0) {
			@Override
		...
		btnAdd.addActionListener(e -> {
			...
			JPanelAddPosition panelAddStatus = new JPanelAddPosition(this);
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		private String action;
		private JPanelPosition parent;
		private int row;

		public ButtonEditor(JCheckBox checkBox, String action, JPanelPosition parent) {
			super(checkBox);
			this.action = action;
			this.parent = parent;
			button = new JButton();
			...
			button.addActionListener(e -> {
				...
				if (action.equals("Update")) {
					...
					if (position != null) {
						...
						JPanelAddPosition panelAddStatus = new JPanelAddPosition(parent);
						panelAddStatus.setPositionData(position);
					...
					int confirm = JOptionPane.showConfirmDialog(parent, "Delete this position?", "Confirm",
							JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						boolean deleted = parent.model.delete(id);
						if (deleted) {
							JOptionPane.showMessageDialog(parent, "position deleted successfully");
							parent.load();
						} else {
	...
	void load() {
		...
		for (Position position : model.findAll()) {
			m.addRow(new Object[] { stt++, position.getTitle(), position.getId(), "Update", "Delete" });
		}
	}
	...
	private boolean isDuplicate(String title, Integer ignoreId) {
		for (Position position : model.findAll()) {
			if (position.getTitle().equalsIgnoreCase(title)) {
				if (ignoreId == null || position.getId() != ignoreId) {
					return true;
				}
			}
		}
	...
	private void searchAndLoad(String keyword) {
		...
		for (Position postion : model.findAll()) {
			if (postion.getTitle().toLowerCase().contains(keyword.toLowerCase())) {
				m.addRow(new Object[] { stt++, postion.getTitle(), postion.getId(), "Update", "Delete" });
				found = true;
			}
		}

		if (!found) {
			m.addRow(new Object[] { "", "No matching positions found", null, "", "" });
