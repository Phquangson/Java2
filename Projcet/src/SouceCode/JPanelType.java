...
public class JPanelType extends JPanel {
...
//		add(lblTitle, BorderLayout.NORTH);
		...
		formPanel
				.setBorder(new TitledBorder(null, "Search Type", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		formPanel.setBackground(Color.WHITE);

		txtTitle = new JTextField(15);
		JButton btnAdd = new JButton("Add");
		btnAdd.setIcon(new ImageIcon(JPanelType.class.getResource("/resources/icon-add.png")));
		styleButton(btnAdd);
		...
		btnClear.setIcon(new ImageIcon(JPanelType.class.getResource("/resources/icon-trash.png")));
		...
		DefaultTableModel modelTable = new DefaultTableModel(new Object[] { "No.", "Type", "ID", "Update", "Delete" },
				0) {
			@Override
		...
		btnAdd.addActionListener(e -> {
			JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Add Type", true);
			dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

			dialog.setUndecorated(true);

			JPanelAddType panelAddStatus = new JPanelAddType(this);
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		public ButtonEditor(JCheckBox checkBox, String action, JPanelType parent) {
			super(checkBox);
			this.action = action;
			this.parent = parent;
			button = new JButton();
			...
			button.addActionListener(e -> {
				...
				if (action.equals("Update")) {
					Type type = TypeModel.findById(id);
					if (type != null) {
						JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(parent), "Update type",
								true);
						dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

						dialog.setUndecorated(true);

						JPanelAddType panelAddStatus = new JPanelAddType(parent);
						panelAddStatus.setTypeData(type);
					...
					int confirm = JOptionPane.showConfirmDialog(parent, "Delete this type?", "Confirm",
							JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						boolean deleted = parent.model.delete(id);
						if (deleted) {
							JOptionPane.showMessageDialog(parent, "Type deleted successfully");
							parent.load();
						} else {
	...
	void load() {
		...
		m.setRowCount(0);
		int stt = 1;
		for (Type type : model.findAll()) {
			m.addRow(new Object[] { stt++, type.getTitle(), type.getId(), "Update", "Delete" });
		}
	}
	...
	private boolean isDuplicate(String title, Integer ignoreId) {
		for (Type type : model.findAll()) {
			if (type.getTitle().equalsIgnoreCase(title)) {
				if (ignoreId == null || type.getId() != ignoreId) {
					return true;
				}
			}
		}
	...
	private void searchAndLoad(String keyword) {
		...
		for (Type type : model.findAll()) {
			if (type.getTitle().toLowerCase().contains(keyword.toLowerCase())) {
				m.addRow(new Object[] { stt++, type.getTitle(), type.getId(), "Update", "Delete" });
				found = true;
			}
		}
		...
		if (!found) {
			m.addRow(new Object[] { "", "No matching types found", null, "", "" });
